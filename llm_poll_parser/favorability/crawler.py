...
from .extract import expressers_rate_row, run_extractor  # noqa: E402
...
def process_question(meta, domanda, text, client=None, llm_enabled=True,
    ...
    from .extract import guard_rows
...
#     uv run python -m llm_poll_parser.favorability.crawler crawl --min-date 01/07/2025
#     uv run python -m llm_poll_parser.favorability.crawler reprocess
#     uv run python -m llm_poll_parser.favorability.crawler average
#     uv run python -m llm_poll_parser.favorability.crawler plot
#     uv run python -m llm_poll_parser.favorability.crawler daily
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="favorability v2 pipeline: crawl / reprocess / average / plot / daily")
